...
class Trainer:
    ...
    def sample_internal(self, step, interpolate=True):     
        persons = []
        x_p_random  = np.random.randint(0,self.gcn.p_dim,self.test_groups)
        # interpolate: False, each emotion generate
        x_e_orders  = [range(self.gcn.e_dim)]*self.test_groups
        # interpolate: True, choose 5 pair emotions <random1, random2> to interpolate ti systhesize new image
        x_e_random1 = np.random.randint(0,self.gcn.e_dim,self.test_groups)
        x_e_random2 = np.random.randint(0,self.gcn.e_dim,self.test_groups)
        for i in range(self.test_groups):
            if interpolate:
                batch_persons = self.gcn.generate_images(x_p_random[i], x_e_random1[i], x_e_random2[i], interpolate)
                persons.append(batch_persons)
            else:
                for emotions in x_e_orders[i]:
                    batch_persons = self.gcn.generate_images(x_p_random[i], emotions, None, interpolate)
                    persons.append(batch_persons)
        
        persons = np.array(persons).reshape(-1,self.gcn.x_dim,self.gcn.x_dim,3)
        columns = 3 if interpolate else self.gcn.e_dim
        save_image(persons, columns, 'data/output', step)
   
    def train(self):
        # initialize global variables
        with self.gcn.session.as_default():
            self.gcn.session.run(tf.global_variables_initializer())
            coord = tf.train.Coordinator()
            thread = tf.train.start_queue_runners(coord=coord)
            
            for step in trange(self.max_step):
                # run optimizer
                self.gcn.session.run(self.gcn.train_items)
                if step % self.log_step == 0:
                    results = self.gcn.session.run(self.gcn.fetch_dict)
                ...
                if step % self.save_step == 0:
                    ...
                    self.gcn.saver.save(self.gcn.session, 'model/checkpoint/gcn.ckpt')
                if step % self.gcn.lr_decay_steps == 0 and step !=0:
                    self.gcn.session.run(self.gcn.lr_update)
                    #pass
