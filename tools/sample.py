...
class Sampler:
    ...
    def sample(self, visualized=True):
        ...
        self.gcn.saver.restore(self.gcn.session, ckpt.model_checkpoint_path)
        ...
        for (key,value) in self.name_label_dict.items():
            ...
            persons = []
            x_e_random1 = np.random.randint(0,self.gcn.e_dim,self.sample_nums)
            x_e_random2 = np.random.randint(0,self.gcn.e_dim,self.sample_nums)
            for idx in range(self.sample_nums):
                batch_persons = self.gcn.generate_images(value, x_e_random1[idx], x_e_random2[idx])
                persons.append(batch_persons)
	    if visualized:
            	persons = np.array(persons)#.reshape(-1, 6, self.gcn.x_dim,self.gcn.x_dim,3)
