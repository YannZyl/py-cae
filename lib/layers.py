...
def add_conv2d(inputs, num_outputs, kernel_size, stride=1, padding='VALID', activation_fn=None, weights=None, bias=None, trainable=True):
    weights_initializer = initializers.xavier_initializer()
    bias_initializer = init_ops.zeros_initializer()
    if weights is not None:
        weights_initializer = init_ops.constant_initializer(weights, tf.float32)
    if bias is not None:
        bias_initializer = init_ops.constant_initializer(bias, tf.float32)
...
def add_deconv2d(inputs, num_outputs, kernel_size, stride=1, padding='SAME', activation_fn=None):
...
def add_lrelu(x, negatice_slop=0.1):
    return tf.maximum(x, x*negatice_slop)

def add_fc(x, num_outputs, activation_fn=None, weights=None, bias=None, trainable=True):
    weights_initializer = initializers.xavier_initializer()
    bias_initializer = init_ops.zeros_initializer()
    if weights is not None:
        weights_initializer = init_ops.constant_initializer(weights, tf.float32)
